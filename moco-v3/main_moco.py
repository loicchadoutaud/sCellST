...
import vits
from data_utils import DistributedBatchSamplerSameDataset
...
parser.add_argument(
    "--list_slides", nargs="+", help="list of slides to use for training."
)
...
def main(args):
    ...
    with open(os.path.join("moco-v3", "config.json"), "w") as f:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            model.load_state_dict(checkpoint["state_dict"])
    checkpointdir = os.path.join("moco-v3", "checkpoints", args.tag)
    os.makedirs(checkpointdir, exist_ok=True)

    save_dir = os.path.join("moco-v3", "best_model")
    os.makedirs(save_dir, exist_ok=True)

    # Load normalization json file from args.output_dir + args.normalization
    mean_std_path = os.path.join(save_dir, f"moco_{args.tag}_model_best_mean_std.json")
    ...
    augmentation = [
        transforms.RandomApply(
            [transforms.ColorJitter(0.6, 0.7, 0.5, 0.2)], p=0.8  # not strengthened
        ),
    ...
    train_dataset = create_dataset(
        ...
        moco.loader.TwoCropsTransform(
            transforms.Compose(augmentation), transforms.Compose(augmentation)
        ),
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        avg_loss = train(
            train_loader, model, optimizer, scaler, summary_writer, epoch, args
        )
    ...
    shutil.copyfile(
        ...
        os.path.join(save_dir, f"moco_{args.tag}_model_best.pth.tar"),
    )
