...
def visualize_instances_dict(
    ...
    for idx, [inst_id, inst_info] in enumerate(inst_dict.items()):
        ...
        if draw_dot:
            ...
            overlay = cv2.circle(overlay, inst_centroid, 10, (255, 0, 0), -1)
    return overlay


def visualize_instance_slides(input_image, centroid_list_wsi, type_list_wsi, type_colour, ds_level) -> Tuple[ndarray, ndarray]:
    overlay = np.copy((input_image))
    black_overlay = np.zeros_like((input_image))

    for idx in range(len(centroid_list_wsi)):
        if type_list_wsi[idx] != 0:
            inst_centroid = tuple([int(v) // (2 ** ds_level) for v in centroid_list_wsi[idx]])
            inst_colour = type_colour[str(type_list_wsi[idx])][1]
            overlay = cv2.circle(overlay, inst_centroid, 3, inst_colour, -1)
            black_overlay = cv2.circle(black_overlay, inst_centroid, 3, inst_colour, -1)

    return overlay, black_overlay
