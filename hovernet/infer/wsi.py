...
from plot_manager import get_file_handler
...
class InferManager(base.InferManager):
    ...
    def process_single_file(self, wsi_path, msk_path, output_dir):
        ...
        self.proc_mag = self.wsi_handler.find_slide_magnification()
        ...
        if msk_path is not None and os.path.isfile(msk_path):
            ...
            log_info(
                f"WARNING: No mask found, generating mask via thresholding at {mask_level}!"
            )
            self.wsi_mask = np.array(make_auto_mask(self.wsi_handler.file_ptr, mask_level) > 0, dtype=np.uint8)
        ...
        if self.save_thumb:
            wsi_thumb_rgb = self.wsi_handler.get_full_img(read_mag=mask_level)
            cv2.imwrite(
